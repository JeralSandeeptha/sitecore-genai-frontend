...
export function MessageList({ messages, isStreaming, error, onRetry, isLoaded }: MessageListProps) {
  ...
  return (
    <div
      ...
      className="absolute inset-0 space-y-4 px-6 pt-16 pb-32 border-none overflow-y-auto"
      role="log"
    ...
    >
      ...
      {messages.length === 0 && !error && !isStreaming && (
        <div className="flex flex-col justify-center items-center h-full text-stone-400 text-center">
      ...
      {error && (
        <div
          className="flex items-center gap-3 bg-red-50 p-4 border border-red-200 rounded-xl"
          role="alert"
          style={{
            boxShadow:
        ...
        >
          ...
          <div className="flex-1">
            <p className="font-medium text-red-800 text-sm">Something went wrong</p>
          ...
          <Button
            ...
            size="sm"
            onClick={onRetry}
            className="font-medium hover:scale-105 transition-all gradient-red-purple-text hover:gradient-red-purple-text"
            aria-label="Retry sending message"
          >
            <RefreshCw className="mr-1 w-4 h-4" aria-hidden="true" />
            Retry
          </Button>
        </div>
      )}
